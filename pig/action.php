...
// 1. Fetch active thresholds AND bypass rules from settings_rule table
$rule_query = "SELECT temperature_on, temperature_off, humidity_on, humidity_off, heater_on, heater_off,
...
if ($rule_result && $rule_result->num_rows > 0) {
    ...
    // Default fallback thresholds & bypass modes
    $rule = [
        'temperature_on'    => '30.0',
...
$light_val      = isset($_POST['light_value']) ? floatval($_POST['light_value']) : 0.0; // Sensor value or digital state (0/1)
...
$heater_off = floatval($rule['heater_off'] ?? 25.0);

// 2. Fetch Previous Machine Status (To maintain consistent state and hysteresis)
$prev_fan     = 'off';
$prev_exhaust = 'off';
$prev_pump    = 'off';
$prev_heater  = 'off';
...
// ==========================================
// 3. Process Cooling Fan Control (GPIO 16)
// ==========================================
...
// ==========================================
// 4. Process Exhaust Fan Control (GPIO 27)
// ==========================================
...
// ==========================================
// 5. Process Pump / Humidity Logic (GPIO 17)
// ==========================================
...
// ==========================================
// 6. Process Heating Lamp Logic (GPIO 26)
// ==========================================
...
if ($light_bypass === 'FORCE_ON') {
    ...
    $new_light_status = $prev_light;
    // Automatic sensor control (turn ON when dark / sensor triggered)
    if ($light_val == 1) { 
        $new_light_status = 'on';
    } else if ($light_val == 0) {
        $new_light_status = 'off';
...
// ==========================================
// 8. Update Machine Status in Database
// ==========================================
...
// ==========================================
// 9. Log to `machine_logs` ONLY when a state changes
// ==========================================
...
// Return JSON response containing all active devices
echo json_encode([
